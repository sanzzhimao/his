...
public class PatientEXaminationDao implements IPatientExaminationDao {
    Connection con=null;
    @Override
    ...
    public List<MedicalTecTest> selectCheckAppley(String registID) throws SQLException {
        String sql="select  c.id,f.itemname,c.position,c.num,f.format,f.price,c.state from checkapply c,fmeditem f where c.registid=" +
                ...
                "and c.recordtype=1 and c.itemid=f.id";
        PreparedStatement psmt=con.prepareStatement(sql);
    ...
    /**
     * @Author lym
     * @Description：用过检查/检验表的id找到检查的信息
     * @Param [id]
     * @return vo.MedicalTecTest
     **/
    @Override
    public MedicalTecTest selectCheckAppley(int id) throws SQLException {
        String sql="select  c.id,f.itemname,c.position,c.num,f.format,f.price,c.state from checkapply c,fmeditem f where c.id=? and c.recordtype=1 and c.itemid=f.id";
        PreparedStatement psmt=con.prepareStatement(sql);
        psmt.setInt(1,id);
        ResultSet rs=psmt.executeQuery();
        MedicalTecTest medicalTecTest=null;
    ...
    public void changeCheckApplyStateById(int id) throws SQLException {
        String sql="update CheckApply set state=5 where id=? and recordtype=1";
        PreparedStatement psmt=con.prepareStatement(sql);
        psmt.setInt(1,id);
        psmt.executeUpdate();
        JdbcUtil.release(null,psmt,null);
